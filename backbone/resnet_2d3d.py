...
class ResNet2d3d_full(nn.Module):
    def __init__(self, block, layers, track_running_stats=True, simplified=False):
        super(ResNet2d3d_full, self).__init__()
        self.simplified = simplified
        self.inplanes = 64
        self.track_running_stats = track_running_stats
        bias = False
        # change channel from 3 to 1 , kernel from 7 to 5
        self.conv1 = nn.Conv3d(
            1, 64, kernel_size=(1, 5, 5), stride=(1, 2, 2), padding=(0, 3, 3), bias=bias
        )
    ...
    def _make_layer(self, block, planes, blocks, stride=1, is_final=False):
        ...
        if is_final and self.simplified:
            layers.append(nn.Conv3d(planes * block.expansion, 128, kernel_size=1))
...
def resnet18_2d3d_simplified(**kwargs):
    """Constructs a simplified ResNet-18 model."""
    model = ResNet2d3d_full(
